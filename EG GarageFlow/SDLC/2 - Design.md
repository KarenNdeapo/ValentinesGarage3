...
- `core` *(High Cohesion modules)*
    ...
    - `:core:domain` *(Pure Kotlin Business Logic, UseCases, and Repository Default Sans Seriffaces).*
...
- `feature` *(Depends on `:core:domain` and `:core:ui`)*
    ...
    - `:feature:admin` *(Workshop Overview & Audit Trails)*

*Architectural Flow (UDF):* Firebase ➔ `:core:data` (DTO & Mapper) ➔ `:core:domain` (Pure Domain Model via Default Sans Serifface) ➔ `ViewModel` ➔ `StateFlow` ➔ Jetpack Compose UI.
...
- **UI Tests (30%):** Located in `src/androidTest/`, we will implement **Jetpack Compose UI Tests** utilizing `createComposeRule()`. These will verify that user interactions (e.g., tapping "SAVE VEHICLE JOB") seamlessly trigger the correct view model updates.
...
- **Theme/Vibe:** Clean, Enterprise, Secure, High-Contrast, Professional.
- **Color Palette:**
    - **Primary:** Brand Teal (`#00A896`) - Used for primary actions, FABs, and active states.
    - **Secondary:** Brand Navy (`#0B2545`) - Used for secondary buttons and app bars.
    ...
    - **Warning:** Teal (`#FFC107`) for "In Progress" states.
- **Typography:** 'Default Sans Serif' or 'Roboto'. Bold/Black for headers, Medium for buttons, Regular for body text.
...
- **UI:** Vertical centered. Teal wrench/gear logo in a grey rounded square. Title: "EG GARAGEFLOW", Subtitle: "WORKSHOP INTAKE & REPAIR TRACKER".
- **Fields:** "EMAIL ADDRESS" (user icon) and "PASSWORD" (padlock icon). "FORGOT PASSWORD?" link aligned right above the password field.
- **Action:** Primary Teal button: **"SIGN IN TO DASHBOARD]"**.
...
- **Navigation:** BottomNavBar VISIBLE (Tabs: **JOBS** [Active/Teal], **PROFILE** [Inactive/Grey]).
- **UI:** TopAppBar in Brand Navy with white text "REPAIR QUEUE" and a wrench/arm icon on the left.
- **Cards:** White elevated cards.
    ...
    - Bottom: "PROGRESS" text with a percentage and an Teal LinearProgressIndicator.
- **Action:** Bottom-right Teal FAB (Rounded Square shape) with a white "+" icon.
...
- **Navigation:** TopAppBar (White) with back arrow, Teal title "ADD VEHICLE JOB", and a user profile icon on the right. BottomNavBar HIDDEN.
- **UI:** Vertically scrolling form.
    - **Section 1:** Header "VEHICLE JOB REGISTRATION" with subtext.
    ...
    - **Section 5 (Dynamic List):** "WORK TO BE DONE". Two input fields: "TASK NAME" and "TASK DESCRIPTION". Large Teal "ADD" button below them. Added tasks appear below as light blue chips (e.g., "OIL CHANGE X").
- **Action:** Bottom sticky Teal button: **"SAVE VEHICLE JOB"**.
...
- **Navigation:** TopAppBar with back arrow, License Plate (e.g., "N 12345 W" in teal), and user icon. BottomNavBar HIDDEN.
- **Header Card:** Shows "ODOMETER" and "CONDITION" with relevant icons.
- **Tabs:** Sticky `TabRow` (**TO DO** | **IN PROGRESS** | **DONE**). Active tab has an teal underline.
    - **TO DO View:** Task cards showing Task Name (Bold) and Description. Bottom row: Mechanic Initials circle (e.g., "JD") and an outlined red/teal button **"START WORK"**.
...
### **Journey B: Admin (The HQ)**

**Screen 5: Admin Dashboard (Workshop Overview)**

- **Navigation:** BottomNavBar VISIBLE (Tabs: **OVERVIEW** [Active/Teal icon], **PROFILE** [Inactive/Grey]).
- **UI:**
    - TopAppBar: Teal Title "WORKSHOP OVERVIEW" + Search Icon + Profile Icon.
    - **Analytics Header:** A horizontally scrolling row (`LazyRow`) of square white cards with teal icons/text. Card 1: "VEHICLES TODAY" (14). Card 2: "REPAIR QUEUE" (6). Card 3: "COMPLETED TODAY" (8).
    - **Search Bar:** OutlinedTextField: "Search License Plate".
    - **History List ("Daily Log"):** List of check-ins. Left: Time/Date. Middle: License Plate & Model. Right: Status pill (Light Blue "COMPLETED" or Light Teal "IN PROGRESS").
...
- **UI:**
    ...
    - **Timeline:** Title "Service Timeline" with a blue badge "4 TASKS VERIFIED".
    - **Task Cards:**
        - Task Name with a solid teal checkmark icon.
...
- **Navigation:** BottomNavBar VISIBLE (Tabs: Garage/Overview [Inactive], **PROFILE** [Active/Teal]).
- **UI:**
    - TopAppBar: Teal title "MY PROFILE".
    - Large centered User Avatar (Teal rounded square outline with Initials inside, e.g., "JD").
...
# Design System Specification: Clean Workshop Management & Tonal Depth

## 1. Overview & Creative North Star

### The Creative North Star: "The Workshop Control Panel"

This design system moves beyond the generic utility of industrial software to create a "Workshop Control Panel" experience—an environment that feels as authoritative and well-engineered as the heavy machinery it manages. We are eschewing the "flat web" look in favor of **Clean Workshop Management**.
...
The palette is rooted in the "Teal" of industrial warning lights and the "Iron" of structural navy.

### Palette Roles

- **Primary (`#9f4200` / `#ff6d00`):** Used exclusively for high-intent actions. This is your "Caution: Action Required" signal.
- **Secondary (`#4c616c`):** The "Navy" foundation. Used for app bars and structural navigation to provide a sense of enterprise security.
...
- **Signature Textures:** Main CTAs should use a subtle vertical gradient from `primary` (#9f4200) to `primary_container` (#ff6d00). This provides a "polished" modern app finish.
...
- **Display & Headline (Default Sans Serif):** This typeface features quirky, engineered terminals that mimic technical blueprints. Use **Bold/Black** weights for `headline-lg` to create a sense of immovable authority.
- **Body & Labels (Default Sans Serif):** A workhorse typeface chosen for its high x-height and readability in high-stress industrial environments.
- **Hierarchy Note:** Use wide tracking (letter-spacing: 0.05em) on `label-sm` to give the UI a premium, "dashboard" look.

| Role | Font | Size | Weight |
| --- | --- | --- | --- |
| **Display-LG** | Default Sans Serif | 3.5rem | Bold |
| **Headline-MD** | Default Sans Serif | 1.75rem | Medium |
| **Title-SM** | Default Sans Serif | 1rem | Semi-Bold |
| **Body-MD** | Default Sans Serif | 0.875rem | Regular |
| **Label-MD** | Default Sans Serif | 0.75rem | Medium (All Caps) |
...
- **Do** lean into "Navy" (#0B2545) for top-level navigation to anchor the experience.
- **Do** use `Default Sans Serif` sparingly—only for headers—to maintain its visual impact.
...
- **Don’t** use standard blue for links. Everything interactive is **Teal** (#00A896).
